...
REGISTRY ?= quay.io
REPOSITORY ?= $(REGISTRY)/redhat-cop/resource-locker-operator
...
export GITHUB_PAGES_REPO ?= redhat-cop/resource-locker-operator
export HELM_CHARTS_SOURCE ?= charts
export HELM_CHART_DEST ?= $(GITHUB_PAGES_DIR)

LDFLAGS := "-X github.com/redhat-cop/resource-locker-operator/version.Version=$(VERSION) \
	-X github.com/redhat-cop/resource-locker-operator/version.Vcs=$(BUILD_COMMIT) \
	-X github.com/redhat-cop/resource-locker-operator/version.Timestamp=$(BUILD_TIMESTAMP) \
	-X github.com/redhat-cop/resource-locker-operator/version.Hostname=$(BUILD_HOSTNAME)"
...
manager: generate fmt vet
	go build -o build/_output/bin/resource-locker-operator  -ldflags $(LDFLAGS) github.com/redhat-cop/resource-locker-operator/cmd/manager

# Build manager binary
manager-osx: generate fmt vet
	GOOS=darwin go build -o build/_output/bin/resource-locker-operator -ldflags $(LDFLAGS) github.com/redhat-cop/resource-locker-operator/cmd/manager
